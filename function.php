...
function pageHeader($view){
    switch($view){
        ...
        case 'tuyen-dung':
            if(isset($_GET['id'])){
                $tab=mysql_query("select title,meta_keyword,meta_description from career where id=".$_GET['id']);
                $row=mysql_fetch_object($tab);
                $pageTitle = $row->title;
                $pageDescription = $row->meta_description;
                $pageKeyword = $row->meta_keyword;
...
function career()
{
    if(isset($_GET['id'])) return career_one();
    else return career_all();
}
function career_all()
{
    $str='
    ...
    <div class="head-title">
        <span>
            <b class="fa fa-calendar"></b> Tuyển dụng
        </span>
    </div>
    ';
    $str.='
    <ul class="news-list">
    ';
    $sql="select id,title,dates,sum,img from career where active=1 order by id desc";
    $tab=mysql_query($sql);
    $count=mysql_num_rows($tab);
    $page=isset($_GET['page'])?intval($_GET['page']):1;
    $start=($page-1)*limit_record;
    ...
    while($row=mysql_fetch_object($tab))
    {
        $str.='
        <li class="clearfix">
            <a href="'.myWeb.'tuyen-dung/'.slug($row->title).'-i'.$row->id.'.html">
            <img src="'.webPath.$row->img.'"/>
            <div>
                <h2>'.$row->title.'</h2>
                <em>'.date("d.m.Y",strtotime($row->dates)).'</em>
    ...
    $pg->paginationcss = "my-pagination";
    $pg->paginationstyle = 1; // 1: advance, 0: normal
    $pg->defaultUrl = myWeb."index.php?view=tuyen-dung";
    $pg->paginationUrl = myWeb."index.php?view=tuyen-dung&page=[p]";
    $str.='<div class="text-center">'.$pg->process().'</div>';
    return $str;
}
function career_one()
{
    ...
    <div class="breadcum">
     <a href="'.myWeb.'"><b class="fa fa-home"></b> Trang chủ</a>
     <a href="'.myWeb.'tuyen-dung.html">Tuyển dụng</a>   
    </div>
    ';
    $tab=mysql_query("select title,content from career where id=$id");
    $row=mysql_fetch_object($tab);
    $str.='
    <div class="head-title">
        <span>
    ...
    <ul class="news-list">
    ';
    $tab=mysql_query("select id,title,dates,sum,img from career where active=1 and id<>$id order by id desc limit 3");
    while($row=mysql_fetch_object($tab))
    {
        $str.='
        <li class="clearfix">
